...
def draw_trajectory(screen, display_width, display_height, asteroid):
    DISPLAY_SIZE = 1080
    ...
    start_pos = (x - dir_x * scale, y - dir_y * scale)
    end_pos   = (x + dir_x * scale, y + dir_y * scale)

    # clip against the square
    rect = pygame.Rect(left, top, square_size, square_size)
    clipped = rect.clipline(start_pos, end_pos)
    
    pygame.draw.line(screen, (100, 100, 100), clipped[0], clipped[1], 1)
    ...
    # Draw asteroid
    pygame.draw.circle(screen, (0, 255, 0), (int(asteroid_x), int(asteroid_y)), 1)